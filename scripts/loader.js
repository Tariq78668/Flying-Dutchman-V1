...
function allBeverages() {
  ...
  for (i = 0; i < DB2.spirits.length; i++) {
    collector.push([
      DB2.spirits[i].nr,
      DB2.spirits[i].articleid,
      DB2.spirits[i].name,
      DB2.spirits[i].catgegory,
      DB2.spirits[i].priceinclvat,
    ]);
  }
  //
  return collector;
...
function beverageTypes() {
  var types = [];
  for (i = 0; i < DB2.spirits.length; i++) {
    addToSet(types, DB2.spirits[i].catgegory);
  }
  return types;
}
...
// [order_list : [order1:[item1, item2]], order2:[item1, item2]..]
//Data structure
...
function addToCart(item) {
  ...
  var isNewItem = currentCart.find((x) => x.nrId === item.nrId);
  if (!isNewItem) {
